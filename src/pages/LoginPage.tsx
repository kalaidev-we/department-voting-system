...
export function LoginPage() {
  ...
  // Primary tab: 'student' (Google SSO hero) vs 'staff' (Institutional password login)
  ...
  // Student direct email fallback state
        ...
        <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-14 items-center">
          ...
          <div className="lg:col-span-5 w-full max-w-md mx-auto">
            <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-xl shadow-blue-900/5 border border-slate-200/80 backdrop-blur-sm relative">
              ...
              {/* Primary Segmented Navigation: Student vs Staff */}
              ...
              {(formError || authError) && (
                <div className="mb-4 p-3 rounded-2xl bg-rose-50 border border-rose-200 text-rose-700 text-xs flex items-start gap-2.5 animate-fadeIn">
              ...
              {successMessage && (
                <div className="mb-4 p-3 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs flex items-start gap-2.5 animate-fadeIn">
                  <CheckCircle2 className="w-4 h-4 shrink-0 mt-0.5 text-emerald-600" />
                  <span className="font-medium leading-relaxed">{successMessage}</span>
                </div>
              )}

              {/* TAB 1: STUDENT VOTER (Google OAuth Hero) */}
              {activeTab === 'student' && (
                <div className="space-y-5 text-center">
                  <div>
                    <div className="w-12 h-12 rounded-2xl bg-blue-50 text-brand-600 flex items-center justify-center mx-auto mb-2 border border-blue-100 shadow-xs">
                    ...
                    <p className="text-xs text-slate-500 mt-1 max-w-xs mx-auto leading-relaxed">
                      Sign in with your official college Google account to cast your verified vote.
                    </p>
                  </div>
                  ...
                  <div className="pt-1">
                    <button
                      ...
                      disabled={isLoading}
                      className="w-full h-14 px-4 rounded-2xl bg-white hover:bg-slate-50 text-slate-800 font-bold border-2 border-slate-200 hover:border-brand-500 shadow-sm hover:shadow-md flex items-center justify-between transition-all duration-200 active:scale-[0.98] group cursor-pointer disabled:opacity-60"
                    >
                      ...
                      <div className="flex flex-col text-left flex-1 px-3">
                        ...
                        <span className="text-[10px] text-brand-600 font-semibold font-mono">
                          @{ALLOWED_DOMAIN}
                        </span>
                      </div>
                  ...
                  {/* Student Benefits / Trust Checklist */}
                  <div className="p-3.5 rounded-2xl bg-blue-50/60 border border-blue-100 text-left space-y-2">
                    <div className="flex items-center space-x-2 text-xs text-slate-700 font-medium">
                      <CheckCircle2 className="w-4 h-4 text-brand-600 shrink-0" />
                      <span>One-click voter identity authentication</span>
                    </div>
                    <div className="flex items-center space-x-2 text-xs text-slate-700 font-medium">
                      <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                      <span>Guaranteed cryptographic ballot secrecy</span>
                    </div>
                    <div className="flex items-center space-x-2 text-xs text-slate-700 font-medium">
                      <CheckCircle2 className="w-4 h-4 text-purple-600 shrink-0" />
                      <span>No password setup or registration required</span>
                    </div>
                  </div>

                  {/* Direct Student Email Verification Accordion (Fallback for restrictive networks) */}
                  <div className="pt-1">
                    <button
                      type="button"
                      onClick={() => setShowStudentFallback(!showStudentFallback)}
                      className="text-xs text-slate-500 hover:text-brand-700 font-semibold inline-flex items-center gap-1 cursor-pointer transition-colors"
                    >
                      <span>Trouble with Google pop-up? Verify via Student Email</span>
                      {showStudentFallback ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
                    </button>

                    {showStudentFallback && (
                      <form onSubmit={handleStudentEmailSubmit} className="mt-3 p-3.5 rounded-2xl bg-slate-50 border border-slate-200 text-left space-y-2.5 animate-fadeIn">
                        ...
                        <div className="relative">
                          ...
                          <input
                            ...
                            className="w-full h-10 pl-9 pr-3 bg-white border border-slate-200 rounded-xl text-xs text-slate-900 font-mono font-medium focus:outline-none focus:border-brand-500"
                          />
                        </div>
                        <button
                          type="submit"
                          disabled={isLoading}
                          className="w-full h-9 rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-bold text-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer disabled:opacity-60"
                        >
                          {isLoading ? (
                            <Loader2 className="w-3.5 h-3.5 animate-spin" />
                          ) : (
                            <>
                              <span>Verify & Enter Voting Portal</span>
              ...
              {/* TAB 2: STAFF & ADMINISTRATOR PORTAL */}
              {activeTab === 'staff' && (
                <div className="space-y-4 text-left">
                  <div className="text-center">
                    <div className="w-12 h-12 rounded-2xl bg-slate-100 text-slate-800 flex items-center justify-center mx-auto mb-2 border border-slate-200 shadow-xs">
                    ...
                    <p className="text-xs text-slate-500 mt-1 max-w-xs mx-auto leading-relaxed">
                      Department Coordinators, Election Commissioners & Master Admin
                    </p>
                  </div>

                  {/* Sub-mode switcher: Sign In vs Magic Link */}
                  <div className="flex rounded-xl bg-slate-100 p-1 border border-slate-200/80">
                    <button
                      type="button"
                      onClick={() => { setStaffAuthMode('signin'); setFormError(null); setSuccessMessage(null); }}
                    ...
                    <button
                      type="button"
                      onClick={() => { setStaffAuthMode('magic'); setFormError(null); setSuccessMessage(null); }}
                      className={`flex-1 py-1.5 px-2 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                        staffAuthMode === 'magic'
                          ? 'bg-white text-slate-900 shadow-xs'
                          : 'text-slate-500 hover:text-slate-900'
                  ...
                  <form onSubmit={handleStaffSubmit} className="space-y-3">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          required
                          placeholder="e.g. staff@kpriet.ac.in or skalaiarasu3@gmail.com"
                    ...
                    <button
                      type="submit"
                      disabled={isLoading}
                      className="w-full h-11 px-4 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs shadow-md shadow-slate-900/20 flex items-center justify-center gap-2 transition-all active:scale-[0.98] cursor-pointer disabled:opacity-60"
                    >
                      {isLoading ? (
                        <>
                        ...
                        <>
                          ...
                          <span>
                            {staffAuthMode === 'signin' && 'Sign In to Campus Console'}
                            {staffAuthMode === 'magic' && 'Send Staff Magic Link'}
                          </span>
                        </>
                      )}
                    </button>
