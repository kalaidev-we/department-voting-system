...
export function AdminDashboardPage({ onNavigateTab }: AdminDashboardPageProps) {
  ...
  return (
    <div className="min-h-screen w-full bg-slate-50 flex flex-col select-none antialiased">
      {/* Top Navbar */}
      <header className="w-full bg-white/95 backdrop-blur-md border-b border-slate-200/80 sticky top-0 z-30 px-3.5 sm:px-8 py-3 flex items-center justify-between shadow-2xs">
      ...
      {/* Main Container */}
      <main className="flex-1 w-full max-w-4xl mx-auto px-3.5 sm:px-6 py-5 sm:py-6 space-y-5 sm:space-y-6">
        {/* Welcome Hero Banner */}
        <div className="p-4 sm:p-6 rounded-3xl bg-gradient-to-br from-indigo-900 via-indigo-800 to-slate-900 text-white shadow-xl shadow-indigo-950/20 relative overflow-hidden">
          <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <button
              onClick={() => onNavigateTab('admin_staff_console')}
              className="w-full sm:w-auto text-center justify-center px-4 py-2.5 rounded-2xl bg-white/10 hover:bg-white/20 border border-white/20 text-xs font-bold text-white flex items-center space-x-1.5 transition-all cursor-pointer shrink-0 backdrop-blur-xs"
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3">
          <div className="p-3 sm:p-4 rounded-2xl bg-white border border-slate-200/80 shadow-xs space-y-1">
