    ...
    <div className="min-h-screen w-full bg-slate-50 flex flex-col select-none antialiased">
      ...
      <header className="w-full bg-white border-b border-slate-200 sticky top-0 z-20 px-3.5 sm:px-6 py-2.5 sm:py-3 flex items-center justify-between">
        ...
        <button
          ...
          className="h-8 sm:h-9 px-2.5 sm:px-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold flex items-center space-x-1 sm:space-x-1.5 shadow-xs cursor-pointer shrink-0"
        >
          ...
          <div className="divide-y divide-slate-100">
            {filtered.length === 0 ? (
              ...
              filtered.map((staff) => (
                <div
                  key={staff.id}
                  className="p-3.5 sm:p-4 hover:bg-slate-50 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 sm:gap-3"
                >
                  <div className="flex items-start sm:items-center space-x-3 min-w-0">
                    <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-2xl bg-indigo-100/70 text-indigo-700 font-bold text-xs sm:text-sm flex items-center justify-center shrink-0 border border-indigo-200/60">
                      {staff.name.charAt(0)}
                    </div>
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-1.5">
                        <h4 className="text-xs sm:text-sm font-bold text-slate-900 truncate">{staff.name}</h4>
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 self-start sm:self-center shrink-0">
                    <span className="px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full bg-blue-50 text-blue-700 text-[9px] sm:text-[10px] font-bold border border-blue-200/60 flex items-center gap-1">
                      <Vote className="w-3 h-3 text-blue-600" />
                      Voter Eligible
                    </span>

                    <span className="px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full bg-emerald-50 text-emerald-700 text-[9px] sm:text-[10px] font-bold border border-emerald-200/60 flex items-center gap-1">
